    ...
    <div className="space-y-6 px-5">
      <section className="rewards-hero">
        <div className="flex items-start justify-between">
          ...
          <span className="reward-sparkle"><Sparkles size={15}/> {t('verifiedWork')}</span>
        </div>
        <p className="mt-5 text-sm font-semibold text-white/80">{t('totalPoints')}</p>
        <div className="mt-1 flex items-end gap-2"><strong className="text-6xl font-black leading-none">{data.totalPoints}</strong><span className="pb-1 text-sm font-bold text-white/75">{t('points')}</span></div>
        <div className="mt-5 grid grid-cols-2 gap-3">
          <div className="reward-glass"><Trophy size={18}/><span>{t('yourRank')}</span><strong>#{data.rank || '—'}</strong></div>
          <div className="reward-glass"><Award size={18}/><span>{t('verifiedActions')}</span><strong>{data.transactions.length}</strong></div>
        </div>
        <p className="mt-4 text-[10px] leading-relaxed text-white/65">{t('demoModel')}</p>
      </section>

      <section>
        <div className="mb-3 flex items-center justify-between"><div><h2 className="section-title">{t('topPerformers')}</h2><p className="text-xs text-slate-400">{t('celebratingCare')}</p></div><Crown className="text-amber-500" size={25}/></div>
        <div className="performers-card">
          {topThree.map(worker => <div className={`performer-row rank-${worker.rank}`} key={worker._id}>
            ...
            <div className="text-right"><strong className="block text-lg text-care-600">{worker.totalPoints}</strong><span className="text-[10px] font-bold text-slate-400">{t('points')}</span></div>
          </div>)}
        </div>
      </section>
      ...
      <section>
        ...
        {recent.length ? <div className="space-y-2">{recent.map(transaction => <RewardRow transaction={transaction} key={transaction._id}/>)}</div> : <p className="card p-5 text-sm text-slate-400">{t('noVerifiedPoints')}</p>}
...
export function RewardRow({ transaction }) {
  ...
  return <div className="reward-row">
    ...
    <div className="min-w-0 flex-1"><strong className="block truncate text-sm">{localizeContent(transaction.description,t)}</strong><p className="text-[11px] text-slate-400">{new Date(transaction.createdAt).toLocaleDateString(language)}</p></div>
